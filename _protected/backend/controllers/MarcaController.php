...
class MarcaController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        $providerModelo = new \yii\data\ArrayDataProvider([
            'allModels' => $model->modelos,
        ]);
        return $this->render('view', [
            'model' => $this->findModel($id),
            'providerModelo' => $providerModelo,
        ]);
    }
    ...
    * @return mixed
    */
    public function actionAddModelo()
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('Modelo');
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('action') == 'load' && empty($row)) || Yii::$app->request->post('action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formModelo', ['row' => $row]);
        } else {
            throw new NotFoundHttpException(Yii::t('app', 'The requested page does not exist.'));
        }
    }
